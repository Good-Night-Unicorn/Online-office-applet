...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zhiweixinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','zhiweixinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('zhiweixinxi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusszhiweixinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('zhiweixinxi', this.detail);
				this.$utils.msg('审核成功');
